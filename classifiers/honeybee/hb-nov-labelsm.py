...
Xe = Xe.reshape([-1, data_length, 1])
    #%%
model_sm3, params = run_model(Xt, Xv, Yt, Yv, class_weights=cw, label_smoothing=0.3,
                          batch_size=300, epochs=70, stop_patience=9, learning_rate=0.0005)

evals = evaluate_model(model_sm3, model_sm3.history.history, Xe.astype(float), Ye, classes_short)
...
oom = 3000
tsne = TSNEplot(Xt[:oom], Yt[:oom], model_sm3, classes=classes_short)
tsne.fit()
tsne.plot()

from fplearn.tsne import TSNEplot
oom = 3000
tsnee = TSNEplot(Xe[:oom], Ye[:oom], model_sm3, classes=classes_short)
tsnee.fit()
tsnee.plot()
